...
public class MercuryEventTriggerListener : UIBehaviour, IMoveHandler, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, ISelectHandler, IDeselectHandler, IDragHandler, IScrollHandler, IEventSystemHandler
{
    #region 事件定义
    ...
    #region 事件触发
    public static MercuryEventTriggerListener Get(GameObject go)
    {
        MercuryEventTriggerListener Listener = go.GetComponent<MercuryEventTriggerListener>();
        if (Listener == null)
        {
            Listener = go.AddComponent<MercuryEventTriggerListener>();
        }
        return Listener;
    }
